...
func (d *delegate) NotifyMsg(b []byte) {
	...
	case 'd': // data
		...
		mtx.Lock()
		fmt.Printf("-----NotifyMsg, updates len:%d\n", len(updates))
...
func (d *delegate) GetBroadcasts(overhead, limit int) [][]byte {
	//fmt.Printf("------GetBroadcasts, overhead:%d, limit:%d \n", overhead, limit)
...
func (d *delegate) MergeRemoteState(buf []byte, join bool) {
	fmt.Printf("-----MergeRemoteState, buf:%s, join:%v \n", string(buf), join)
	if len(buf) == 0 {
		return
	}
	if !join {
	...
	fmt.Printf("MergeRemoteState, m:%v \n", m)
	mtx.Lock()
	for k, v := range m {
		items[k] = v
	}
