...
import (
	...
	"github.com/jursonmo/practise/pkg/fast_l2fwd"
)
...
func main() {
	...
	if op == "del" {
		...
		err := fast_l2fwd.Detach(inIfindex)
		if err != nil {
			fmt.Println(err)
		}
		return
	...
	program, err := fast_l2fwd.NewFastL2fwdProgram(ipproto, nil)
	if err != nil {
		fmt.Printf("err:%v", err)
		return
	}
	...
	if redirectOnly {
		...
		log.Printf("redirectOnly, from inIfindex:%d to outIfindex:%d", inIfindex, outIfindex)
	...
	if err != nil {
		fmt.Printf("AttachProgram err:%v", err)
		return
	}
