...
func main() {
	var indev, outdev string
	var queueID int
	var protocol int64

	log.SetFlags(log.Ldate | log.Ltime | log.Lmicroseconds)

	flag.StringVar(&indev, "indev", "enp1s0", "The network link on which rebroadcast should run on.")
	flag.StringVar(&outdev, "outdev", "enp2s0", "The network link on which rebroadcast should run on.")
	...
	for _, iface := range interfaces {
		if iface.Name == indev {
			Ifindex = iface.Index
		} else if iface.Name == outdev {
			outIfindex = iface.Index
		}
	}
	if Ifindex == -1 || outIfindex == -1 {
	...
	out_xsk, err := xdp.NewSocket(outIfindex, queueID, nil)
	if err != nil {
		fmt.Printf("error: failed to create an XDP socket: %v\n", err)
		return
	}
