...
func NewZapWriter(path string, opts ...zap.Option) (logx.Writer, error) {
	opts = append(opts, zap.AddCallerSkip(callerSkipOffset))
	pcf := zap.NewProductionConfig()
	pcf.OutputPaths = []string{path}
	pcf.ErrorOutputPaths = []string{path}

	logger, err := pcf.Build(opts...)
